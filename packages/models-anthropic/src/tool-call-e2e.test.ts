...
/** Every committed envelope in the store, in order — local copy of core's own test helper, not worth a cross-package test dependency for one file. */
function loggedEnvelopes(store: SessionStore): CommittedEnvelope[] {
  return store
    .events()
    .filter((envelope): envelope is CommittedEnvelope => envelope.form === "committed");
}
...
describe("the real Anthropic port runs a tool call end to end", () => {
  it("commits toolCall and toolResult events and runs the real handler, with no network", async () => {
    ...
    const port = createAnthropicPort(model, fakeClient);
