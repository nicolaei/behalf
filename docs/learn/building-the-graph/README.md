...
Composing steps into a runnable flow — nodes, edges, threads, and the points where a flow waits on
the outside world.
A couple of examples here name a `Profile` (a persona) before that's formally introduced next, in
Describing a flow — treat it as an opaque named persona for now.
