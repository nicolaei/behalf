...
export const audit: Graph = defineGraph("audit", (flow) => {
  const intake = flow.step(outputs((context) => context.inputs[0]));

  const security = flow.step(async (context) => {
    await context.modelCall(securityReviewer);
    return context.output(lastAssistantText(context));
  });
  const performance = flow.step(async (context) => {
    await context.modelCall(performanceReviewer);
    return context.output(lastAssistantText(context));
  });
  const style = flow.step(async (context) => {
  ...
  // #region join
  const merge = flow.step(join((context) => context.inputs));
  security.then(merge);
